...
> Seed (docs markdown ficticios + payments.json): se crea en fase 2; ubicación sugerida
> `apps/server/seed/` (fuera de `src/`, no compila como código de la app).
...
- `seed/docs/`: markdown con secciones — architecture.md, payments.md, sap.md, retries.md,
  troubleshooting.md (+ un doc "envenenado" para la fase de seguridad).
- `payments.json`: payments + payment_events + payment_retries (payment 12345 FAILED,
...
interface LlmProvider {
  ...
  chatWithTools?(req: ChatRequest & { tools: ToolDefinition[] }): Promise<ChatResponse>;
